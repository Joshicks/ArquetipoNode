...
describe('UserFacade Test', () => {
    ...
    describe('Update', () => {
        it('should update the user', async () => {
          ...
          const user: UserTo = await UserFacade.update(1, userTo);
          expect(user.name).equal("Juan Updated");
        });
      
        it('should throw a ParametersError if attributes are missing', async () => {
          let userTo: UserTo = {
            name: "Juan Updated"
          }
          try {
            await UserFacade.update(1, userTo);
          } catch (error: any) {
            expect(error).instanceOf(ParametersError)
            expect(error.message).equal('El atributo email es requerido')
          }
      ...
      describe('Eliminate', () => {
        it('should eliminate the user', async () => {
          await UserFacade.publish_eliminate(1);
          const User: any[] = await UserFacade.findAll();
        ...
        it('should throw an error if the user does not exist', async () => {
          try {
            await UserFacade.publish_eliminate(2);
          } catch (error: any) {
            expect(error).instanceOf(Error)
            expect(error.message).equal('No se pudo eliminar el usuario')
          }
        });
      });
